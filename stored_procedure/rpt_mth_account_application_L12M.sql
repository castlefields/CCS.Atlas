...
GO
CREATE OR ALTER   PROCEDURE [dbo].[rpt_mth_account_application_L12M]
...
SET @report_name = 'Consumer Credit Solutions : ' + COALESCE(@facility_lender, 'All');
    ...
    FROM dbo.get_account_paid_last_4_years_by_facility_code p
        INNER JOIN dbo.get_moment_L12M m ON p.year_month = m.year_month
    WHERE
        (p.facility_used_by_id = COALESCE(@facility_used_by_id, p.facility_used_by_id))
        AND (p.facility_used_by_report_group_id = COALESCE(@report_group_id, p.facility_used_by_report_group_id))
        AND (p.facility_lender = COALESCE(@facility_lender, p.facility_lender))
    GROUP BY
        p.year_month;
    ...
    BEGIN
        ...
        FROM dbo.get_account_application_last_4_years_by_facility_code a
            INNER JOIN dbo.get_moment_L12M m ON a.year_month = m.year_month
        WHERE
            (a.facility_used_by_id = COALESCE(@facility_used_by_id, a.facility_used_by_id))
            AND (a.facility_used_by_report_group_id = COALESCE(@report_group_id, a.facility_used_by_report_group_id))
    ...
    BEGIN
        ...
        WHERE
            (a.facility_used_by_id = COALESCE(@facility_used_by_id, a.facility_used_by_id))
            AND (a.facility_used_by_report_group_id = COALESCE(@report_group_id, a.facility_used_by_report_group_id))
            AND a.facility_lender = @facility_lender
        GROUP BY a.year_month;
    END
