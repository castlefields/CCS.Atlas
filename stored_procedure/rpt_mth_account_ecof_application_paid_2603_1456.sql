CREATE OR ALTER PROCEDURE [dbo].[rpt_mth_account_ecof_application_paid_1456]
...
BEGIN
    ...
    -- Insert into #Ecof with proper grouping
    INSERT INTO #Ecof
        (year_month, year, month, quarter, year_quarter, ecof_count, ecof_advance)
    SELECT
        e.year_month,
        CAST(LEFT(e.year_month, 4) AS INT) AS year,
        CAST(RIGHT(e.year_month, 2) AS INT) AS month,
        (CAST(RIGHT(e.year_month, 2) AS INT) - 1) / 3 + 1 AS quarter,
        LEFT(e.year_month, 4) + 'Q' + CAST(((CAST(RIGHT(e.year_month, 2) AS INT) - 1) / 3 + 1) AS NVARCHAR) AS year_quarter,
    ...
    SELECT
        p.year_month,
        CAST(LEFT(p.year_month, 4) AS INT) AS year,
        CAST(RIGHT(p.year_month, 2) AS INT) AS month,
        (CAST(RIGHT(p.year_month, 2) AS INT) - 1) / 3 + 1 AS quarter,
        LEFT(p.year_month, 4) + 'Q' + CAST(((CAST(RIGHT(p.year_month, 2) AS INT) - 1) / 3 + 1) AS NVARCHAR) AS year_quarter,
    ...
    SELECT
        a.year_month,
        CAST(LEFT(a.year_month, 4) AS INT) AS year,
        CAST(RIGHT(a.year_month, 2) AS INT) AS month,
        (CAST(RIGHT(a.year_month, 2) AS INT) - 1) / 3 + 1 AS quarter,
        LEFT(a.year_month, 4) + 'Q' + CAST(((CAST(RIGHT(a.year_month, 2) AS INT) - 1) / 3 + 1) AS NVARCHAR) AS year_quarter,
    ...
    SELECT
        ...
        m.' + @group_by + ' AS group_by,
        SUM(result.ecof_count) AS ecof_count,
        SUM(result.ecof_advance) AS ecof_advance,
        CASE WHEN SUM(result.ecof_advance) IS NULL THEN NULL ELSE ''£'' + FORMAT(SUM(result.ecof_advance), ''N0'') END AS ecof_advance_text,
        SUM(result.proposed) AS proposed_count,
        SUM(result.accept) AS accept_count,
        SUM(result.accept_val) AS accept_val,
        CASE WHEN SUM(result.accept_val) IS NULL THEN NULL ELSE ''£'' + FORMAT(SUM(result.accept_val), ''N0'') END AS accept_text,
        CASE WHEN SUM(result.accept) > 0 THEN ''£'' + FORMAT(SUM(result.accept_val) / NULLIF(SUM(result.accept), 0), ''N0'') END AS accept_avg_text,
        CASE WHEN SUM(result.accept + result.decline) > 0 THEN CAST(SUM(result.accept) * 100.0 / NULLIF(SUM(result.accept + result.decline), 0) AS NVARCHAR) + ''%'' END AS accept_rate_text,
        ...
        CASE WHEN SUM(result.advance) IS NULL THEN NULL ELSE ''£'' + FORMAT(SUM(result.advance), ''N0'') END AS advance_text,
        CASE WHEN SUM(result.advance_count) > 0 THEN ''£'' + FORMAT(SUM(result.advance) / NULLIF(SUM(result.advance_count), 0), ''N0'') END AS advance_avg_text,
        SUM(result.commission) AS commission_val,
        CASE WHEN SUM(result.commission) IS NULL THEN NULL ELSE ''£'' + FORMAT(SUM(result.commission), ''N0'') END AS commission_text,
        CASE WHEN SUM(result.advance) > 0 THEN SUM(result.commission) / NULLIF(SUM(result.advance), 0) END AS commission_rate_avg,
        CASE WHEN SUM(result.advance) > 0 THEN FORMAT(SUM(result.commission) / NULLIF(SUM(result.advance), 0) * 100, ''N2'') + ''%'' END AS commission_rate_avg_text
    FROM
        #FilteredMoments m
        LEFT JOIN (
        SELECT
