CREATE OR ALTER PROCEDURE [dbo].[rpt_mth_account_application_period]
...
BEGIN
    ...
    -- Final result set - use filtered moments as the base
    SELECT
        @report_name AS report_name,
        m.year_month,
        result.accept_val AS accept_val,
        result.accept AS accept,
        result.decline AS decline,
        result.pending AS pending,
        result.proposed AS proposed,
        result.accept AS accept,
        CASE WHEN result.accept_val IS NULL THEN NULL ELSE '£' + FORMAT(result.accept_val, 'N0') END AS accept_val,
        CASE WHEN result.accept_average IS NULL THEN NULL ELSE '£' + FORMAT(result.accept_average, 'N0') END AS accept_average,
        CASE WHEN result.accept_rate IS NULL THEN NULL ELSE CAST(result.accept_rate AS NVARCHAR) + '%' END AS accept_rate,
        result.advance_count AS advance_count,
        CASE WHEN result.advance IS NULL THEN NULL ELSE '£' + FORMAT(result.advance, 'N0') END AS advance,
        CASE WHEN result.advance_average IS NULL THEN NULL ELSE '£' + FORMAT(result.advance_average, 'N0') END AS advance_average,
        result.commission AS commission,
        CASE WHEN result.commission IS NULL THEN NULL ELSE '£' + FORMAT(result.commission, 'N0') END AS commission_formatted,
        result.commission_rate_avg AS commission_rate_avg,
        CASE WHEN result.commission_rate_avg IS NULL THEN NULL 
             ELSE FORMAT(result.commission_rate_avg * 100, 'N2') + '%' END AS commission_rate_avg_formatted
    FROM
        #FilteredMoments m
        LEFT JOIN (
            SELECT
