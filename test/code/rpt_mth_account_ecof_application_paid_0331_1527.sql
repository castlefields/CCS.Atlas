/****** Object:  StoredProcedure [dbo].[rpt_mth_account_ecof_application_paid_2703_1529]    Script Date: 31/03/2025 12:23:03 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE OR ALTER PROCEDURE [dbo].[rpt_mth_account_ecof_application_paid_0331_1527]
...
BEGIN
    ...
    INSERT INTO #Budget
        (year_month, year, month, quarter, year_quarter, budget)
    SELECT
        b.year_month,
        m.year,
        m.month,
    ...
    WHERE
        (@facility_used_by_id IS NULL OR b.facility_used_by_id = @facility_used_by_id)
        AND (@report_group_id IS NULL OR b.facility_used_by_report_group_id = @report_group_id)
    ...
    FROM
        ...
        FROM
            ...
            LEFT JOIN #Budget b ON a.year_month = b.year_month
        GROUP BY
            a.year_month,
            a.year,
            a.month,
